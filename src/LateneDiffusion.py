...
#         self.file_lengths = []
#         for file in self.file_list:
#             arr = np.load(f"{folder_path}/{file}", mmap_mode='r')
...
#         arr = np.load(
#             f"{self.folder_path}/{self.file_list[file_idx]}",
#             mmap_mode='r'
#         )

#         return arr[local_idx].astype(np.float32) * 25
...
class MyDataset_nocond(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return arr[int(local_idx)].astype(np.float32) * 25
