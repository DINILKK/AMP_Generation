...
if __name__ == "__main__":
    ...
    parser.add_argument("--vae_epoch",                      default=20, type=int )
    parser.add_argument("--vae_batch_size", default=16, type=int)    
    ...
    parser.add_argument("--LatentDiffusion_batch_size", default=16, type=int)    
    parser.add_argument("--LatentDiffusion_num_steps", default=200, type=int)
    parser.add_argument("--LatentDiffusion_shuffle",        default=False,                  type=bool)
    parser.add_argument("--LatentDiffusion_num_workers",    default=2,                      type=int)
    ...
    parser.add_argument("--Generate_batch_num",             default=64             ,type=int )
    ...
    # {'sequence accuracy': 0.9921936854972011, 'sequence confidence': 0.0004545871858095517, 'token accuracy': 0.9993429496509196, 'token confidence': 2.125677299529601e-05, 'bleu_score': 0.998939270668447}
        ...
        # NEW - load your dataset and split
        
        df = pd.read_csv('./dataset/custom_dataset.csv')  # change filename
        df = df[df['SEQUENCE'].str.len() <= 126]
        data_all = df[['SEQUENCE']].dropna().to_numpy()
        data_train, data_val = ms.train_test_split(data_all, test_size=0.2, random_state=42)
        ...
        save_path = './model_finetuned'
        model = DModel().to(device)
        ...
        train_data = MyDataset_nocond(f"./memory/train")
        val_data   = MyDataset_nocond(f"./memory/val")
        # train_sample = DistributedSampler(train_data)
        # val_sample = DistributedSampler(val_data)
        ...
        for epoch in range(num_epochs):
            ...
            for i,data in enumerate(train_run):
                ...
                data = data.to(device)
                opt.zero_grad()
                time_steps, weights = sampler.sample(batch_size=data.shape[0], device_id=device)
            ...
            for i, vdata in enumerate(val_run):
                seq = vdata
                seq = seq.to(device)
                time_steps, weights = sampler.sample(batch_size=seq.shape[0],device_id=device)
                time_steps = time_steps.to(device)
            ...
            if rank == 0:
                ...
                if epoch % 1 == 0:
                    torch.save(model.state_dict(),f"{save_path}/model/model_{epoch}_{train_loss}_{val_loss}.pth")
                print(f"########################################################################################")
        ...
        for _ in tqdm(range(len(file_list))):
            ...
            chunk = 64
            for i in range(int((args.Generate_batch_num * args.Generate_batch_times)/chunk)):
                tem = result[i * chunk : ( i + 1 ) * chunk, : ]
